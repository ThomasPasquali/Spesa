    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(id, utente) {
                ...
                var html = '<div class="utente"><button type="button" class="utente rmUtenteAssociato" id="' + utente.Nome + '"><i class="fa fa-remove rimuovi"></i></button><input class="utente" type="hidden" readonly="readonly" name="utente" value="' + utente.Nome +
...
function checkUtenti(utenti) {
    var html = '';
    $.each(utenti, function(index, utente) {
        html += `<div class="utentecheck"><input type="checkbox" id="${utente.Nome}" value="${utente.Nome}"><label>${utente.Nome}</label></div>`;
    });
    return html;
}
...
export { getIDfromDatalist, createHTMLScontrino, sendRequest, requestUsers, replaceAll, checkUtenti };