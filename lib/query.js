...
module.exports = {
    ...
    getOggettiSupermercato : function(supermercato) {
        return db.query('SELECT ID, Nome, Note FROM oggetti WHERE Supermercato = ? ORDER BY Nome ASC', [supermercato]);
    },
    ...
    getLista : function(listaID) {
        return db.query('SELECT l.ID, l.Nome, s.Nome Supermercato, l.Richiedi_prezzi, l.Gruppo FROM liste l JOIN supermercati s ON s.ID = l.Supermercato WHERE l.ID = ?', [listaID]);
    },
