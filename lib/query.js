...
module.exports = {
    ...
    getDatiUtente: function(user) {
        return db.query('SELECT Password, Ruolo, Preferenze FROM utenti WHERE Nome = ?', [user]);
    },

    /**
     * @param {int} gruppo L'ID del gruppo
     * @returns {Promise} Tutti gli utenti presenti
     */
    getUtentiManacanti: function(gruppo) {
        return db.query('SELECT * FROM spesa.utenti u where u.Nome NOT IN (SELECT g.Utente FROM spesa.gruppi_utenti as g where g.Gruppo = ?)', [gruppo]);
    },
    ...
    getRicetteUtente: function(username) {
        return new Promise(function(resolve, reject) {
            var ricette = {};
            db.query(`SELECT r.ID ricetta, r.Nome nomeRicetta, r.Descrizione descrizioneRicetta, r.Supermercato supermercatoRicetta, CONCAT('{',GROUP_CONCAT(CONCAT('"',o.ID,'": {','"nome":"',o.Nome,'", "note":"',o.Note,'"}')),'}') oggetti  FROM gruppi_utenti gu JOIN gruppi_ricette gr ON gr.Gruppo = gu.Gruppo JOIN ricette r ON r.ID = gr.Ricetta  JOIN oggetti_ricette ogr ON ogr.Ricetta = gr.Ricetta  JOIN oggetti o ON o.ID = ogr.Oggetto  WHERE gu.Utente = ? GROUP BY r.ID`, [username]).then(function(res) {
                for (const r of res)
                    ricette[parseInt(r.ricetta)] = {
                        nome: r.nomeRicetta,
                        descrizione: r.descrizioneRicetta,
                        supermercato: r.supermercatoRicetta,
                        oggetti: JSON.parse(r.oggetti)
                    };
                resolve(ricette);
            }).catch(() => reject());
        });
    ...
    aggiungiOggettoALista: function(oggetto, lista, quantita) {
        console.log(`INSERT INTO liste(Oggetto, Lista, Quantita) VALUES (${oggetto}, ${lista}, ${quantita})`);
    ...
    /**
     * @param {int} idGruppo L'ID del gruppo a cui aggiungere l'utente
     * @param {String} utente Il nome dell'utente da aggiungere
     * @returns {Promise} L'esito della query
     */
    insertUtenteInGruppo: function(idGruppo, utente) {
        return db.query('insert into spesa.gruppi_utenti(Gruppo, Utente) values (?, ?)', [idGruppo, utente]);
    },
    ...
    insertGruppoUtenti: function(nome) {
        return db.query('INSERT INTO gruppi(Nome) VALUES (?)', [nome]);
    },

    insertOggettoRicetta: function(ricetta, oggetto, supermercato) {
        return db.query('INSERT INTO oggetti_ricette(Ricetta, Oggetto, Supermercato) VALUES (?,?,?)', [ricetta, oggetto, supermercato]);
    },
    ...
    updateRicetta: function(id, descrizione) {
        return db.query('UPDATE ricette SET Descrizione = ? WHERE ID = ?', [descrizione, id]);
    },

    deleteUtenteFromGruppo: function(idGruppo, utente) {
        return db.query('DELETE FROM spesa.gruppi_utenti WHERE Gruppo = ? AND Utente  = ?', [idGruppo, utente]);
    },
    ...
    deleteOggettoLista: function(oggetto, lista) {
        return db.query('DELETE FROM oggetti_liste WHERE Oggetto = ? AND Lista = ?', [oggetto, lista]);
    },

    deleteOggettoRicetta: function(oggetto, ricetta) {
        return db.query('DELETE FROM oggetti_ricette WHERE Oggetto = ? AND Ricetta = ?', [oggetto, ricetta]);
    },
